...
- **A stats reset clears** chooses what a reset clears: the current reading (default), the whole rotation set, or every dial. "Every dial" includes dials parked on other pages and profiles, and it never rides on a default gesture; you have to pick it on purpose.
