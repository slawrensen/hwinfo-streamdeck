...
Effectively no. Measured on a live page at a 1-second poll (every number regenerated by `node scripts/perf-report.mjs`; see [PERF.md](https://github.com/slawrensen/hwinfo-streamdeck/blob/main/PERF.md) in the repo):

- **CPU: a fraction of one percent of one core** (~0.2 % long-run average on the live deck; **0.00 %** when its keys aren't on screen, because polling stops entirely).
- **RAM: ~40–48 MB** RSS, stable across externally monitored multi-hour soaks (PERF.md carries the measured slopes).

The parse path is incremental: one poll tick to decode ~516 readings costs about **6 µs** with near-zero allocation. Most of that memory is the Node runtime the Stream Deck app hosts plugins in, not the plugin's own data; the plugin's native bridge is a single 152 KB addon.
