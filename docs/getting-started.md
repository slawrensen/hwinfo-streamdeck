...
Values change as often as HWiNFO polls its sensors, every 2 seconds by default; see [update rate](data-sources.md#update-rate). Shared Memory selections use HWiNFO's reading identity, so reordering the list does not change the selection. Gadget uses source names and reading labels. Switching between providers requires explicit links (since 1.7); see [data sources](data-sources.md).
