...
- If what you want is a page you lay out yourself, with your own multi-reading keys on it, use a [workspace page](workspace-pages): set the key's Press to "Open a workspace page" and it switches to a blank page you fill. A Stream Deck folder does the same job as the app's own feature (right-click a key, Create Folder) and belongs to that one key, but its opener shows a static icon, while a workspace or drill-down key keeps showing a live reading.
...
- **A second Back under your finger, if you ask for it.** Tick **Repeat Back under this key's own cell** on the opener and, when that key's cell maps onto a reading cell of the page, the cell becomes a second Back tile showing the same opener face with the return arrow: tap in, tap out, without moving your hand. The readings flow around it (none are hidden). Off by default ([issue #5](https://github.com/slawrensen/hwinfo-streamdeck/issues/5) testing settled on one movable Back over two fixed ones), and the top-left Back always works regardless.
...
**Tile shows** multiplies those counts without touching the page: the tiles stay put and each carries up to four readings, so a + XL page lists up to 128 readings at once and the paging steps by readings, not tiles. The optional second Back still costs exactly one tile per page.
