...
| **Color numbers by sensor type** | Issue #31 preview only, under Appearance for two-row and three-row views. Off by default. Normal numbers use their reading's type color; labels, units and footer keep their Text styling. Use Text → Theme or Dim with Type accents enabled. Valid Custom Text, including an inherited deck default, overrides sensor colors. Alerts keep priority. Paper, accents off and unknown types keep normal text. Switching views preserves the choice. Not available in stable 1.6.0. |
| **Reading colors** | Issue #31 preview only, under Appearance for two-row and three-row views. Choose **Signal (four hues)** or use each reading's color well, as on the four-reading key. Colors follow reading identity through rotation, reordering and groups; two temperatures can differ. **Auto** resets one reading; **Automatic** resets the listed readings. Unset readings follow the sensor-type option above, or normal text. Chosen colors work with Type accents off and on Paper, remain exact in Text → Theme, and use the existing Dim blend. Valid Custom Text and alerts retain priority. Labels, units, graphs and selection styling stay unchanged. Switching views or removing a reading keeps its saved color. Not available in stable 1.6.0. |
...
Local statistics count observed samples, not repeated held frames. Their average is sample-weighted. Missing readings, stale/unavailable data, native-unit/provider/link changes and explicit resets start a new session. See [collection and gap semantics](data-sources.md#freshness-and-local-history).
