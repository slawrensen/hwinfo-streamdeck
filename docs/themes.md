...
- **Alerts use their own colors and shapes.** Warning and critical keys use global palettes plus a triangle or octagon, so severity has a persistent cue beyond color.
...
- **Dim**: lower-intensity text, with numeric values, units and session statistics kept at a 4.5:1 authored-color contrast floor. The algorithm respects both dark and light backgrounds, including the selected dial row.
