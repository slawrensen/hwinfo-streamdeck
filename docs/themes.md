...
- **The physics sets the sizes.** At a normal desk distance a Stream Deck key is small enough that one canvas pixel covers roughly half an arc minute of your vision. Value digits are sized to stay glanceable; the label is sized for identification, so it earns moderate contrast, never brightness.
...
Every screenshot in these docs is real renderer output regenerated by scripts, never a mockup, and the geometry above is locked by the test suite.
