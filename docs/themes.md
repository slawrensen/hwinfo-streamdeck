...
- **Theme text** *(the default)*: the selected theme's own text colors, exactly as before.
- **Dimmed**: a lower-intensity version of the theme's text. One fixed algorithm blends each text color toward the theme background, so it lands correctly on dark and light themes alike and keeps the value/label/unit hierarchy.
