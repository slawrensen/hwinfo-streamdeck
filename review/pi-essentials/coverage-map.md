...
| Tile plan | tile editor | `detailTiles` : object[] (unknown fields per tile kept) | absent | action | density | mode custom | A | Press → tile editor | per-tile patch (`patchEntry`) | P: lossless tile checks; L: tile legs |
...
| Rotation membership | ticks in the Sensor picker | `rotationKeys` : string[] (non-string entries kept) | [] = the picked sensor's readings | action | picked source | always | E | Reading → Readings to rotate through (checklist) | tick adds/removes one key; never changes `readingKey` | P: "ticking adds to the rotation" |
...
| Rotation groups | Split into groups | `rotationGroups` : object[] (unknown fields per group kept; flat mirror kept) | absent | action | flat set | groups exist | C | Reading → group editor | per-group patch (`patchEntry`) | P: lossless group checks |
...
| Controls preset | Dial gestures & advanced → Controls | `controlPreset` : "legacy"\|"elite"\|"custom" | "legacy" | action | none | always | C | Controls → Controls | select; Elite to Custom copies the map | P: edit "gesture preset" |
