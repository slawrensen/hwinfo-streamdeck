...
const chrome = spawn(CHROME, [
	...
	`--user-data-dir=${path.join(process.env.TEMP ?? process.env.TMPDIR ?? "/tmp", "pi-capture-profile")}`,
	"--hide-scrollbars",
	"about:blank"
], { stdio: "ignore" });
...
try {
	...
	await evaluate(`(() => { let n = 0; for (const tick of document.querySelectorAll('#pickerr-list .hw-row:not([hidden]) input.hw-tick:not(:checked)')) { if (n >= 2) break; tick.click(); n++; } return n; })()`);
	await sleep(500);
