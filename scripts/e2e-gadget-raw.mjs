...
const actions = [
	{ context: "raw-key", action: "reading", device: "keys", controller: "Keypad", settings: { readingKey: reading, decimals: "0" } },
	{ context: "raw-dial", action: "dial", device: "plus", controller: "Encoder", settings: { readingKey: reading, decimals: "0" } },
	{ context: "healthy", action: "reading", device: "keys", controller: "Keypad", settings: { readingKey: "g:Board:Power", decimals: "0" } }
];
const traffic = [];
let socket;
let plugin;
...
server.on("connection", (ws) => {
	...
	ws.on("message", (data) => {
		...
		if (message.event === "getGlobalSettings") send({ event: "didReceiveGlobalSettings", payload: { settings: { source: "gadget", pollIntervalMs: "250", readingLinks: [{ sharedMemory: reading, gadget: "g:Raw fixture:Temperature", unit: "°C", sensorType: 1 }] } } });
		if (message.event === "registerPlugin") {
			for (const row of actions) send({ event: "willAppear", action: `com.lawrensen.hwinfo.${row.action}`, context: row.context, device: row.device, payload: { settings: row.settings, controller: row.controller, coordinates: { column: row.context === "healthy" ? 1 : 0, row: 0 }, isInMultiAction: false } });
		}
	});
});
...
try {
	for (const [slot, row] of [[0, { Sensor: "Raw fixture", Label: "Temperature", Value: "39 °C", ValueRaw: "39" }], [900, { Sensor: "Board", Label: "Power", Value: "50 W", ValueRaw: "50" }]]) {
	...
	for (const context of ["raw-key", "raw-dial", "healthy"]) {
		const svg = frames(0, context).at(-1);
		fs.writeFileSync(path.join(output, `${context}.svg`), svg);
	}
	assert.ok(hasValue("50")(frames(0, "healthy").at(-1)), "healthy neighbor stays available through malformed data and recovery");
