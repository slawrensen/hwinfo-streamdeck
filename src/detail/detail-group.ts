...
import { detailKeysOf, detailModeOf, detailTitleOf, type DetailMode } from "./detail-settings";
...
export type DetailPage = {
	...
	/** Clamped page offset actually shown (multiple of pageSize). */
...
/** Projects one logical page; pure math, safe for any offset. */
export function pageOf(keys: readonly string[], offset: number, pageSize: number): DetailPage {
	const safeSize = Math.max(1, Math.trunc(pageSize));
	const start = clampOffset(keys.length, offset, safeSize);
	const slots: (string | undefined)[] = [];
	for (let i = 0; i < safeSize; i++) {
		slots.push(keys[start + i]);
	}
	const shown = Math.min(safeSize, Math.max(0, keys.length - start));
	const rangeText = keys.length === 0 ? "0 / 0" : `${start + 1}-${start + shown} / ${keys.length}`;
	return {
		slots,
		offset: start,
		hasPrevious: start > 0,
		hasNext: start + safeSize < keys.length,
		rangeText
	};
}
