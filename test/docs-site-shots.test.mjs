...
async function inspectLaunch({ port, exited = false, spawnError = false } = {}) {
	...
	const launches = [];
	const reads = [];
	const child = Object.assign(new EventEmitter(), { pid: 123, exitCode: exited ? 1 : null, signalCode: null, kill() {} });
	let failure;
	try {
		await vm.runInNewContext(`(async () => { ${source}\n })()`, {
			...
			fetch: async (url) => { fetches.push(url); return { json: async () => [] }; },
			WebSocket: class { constructor() { throw new Error("no page target exists in this fixture"); } },
	...
	return { failure, profile, fetches, cleanups, launches, reads };
}
...
test("docs screenshots fail before endpoint access when Chrome exits or fails to spawn", async () => {
	for (const options of [{ exited: true }, { spawnError: true }]) {
